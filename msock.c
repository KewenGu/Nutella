...
int msockcreate(int type, char *address, int port) {
  ...
  if (type == CLIENT) {
    ...
    if ((ret=setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) < 0) {
      perror("setsockopt reuseaddr");
      return ret;
    }

    if ((ret=bind(sock, (struct sockaddr *)&adr[sock], sizeof(adr[sock]))) < 0) {
      perror("bind");
      return ret;
    }
    ...
    if ((ret=setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq))) < 0) {
